...
class FCNNModel(otbtf.ModelBase):
    ...
    def get_outputs(self, normalized_inputs):
        ...
        cv5 = conv(cv4, 512, "conv5")
        cv5 = BatchNormalization()(cv5)
        cv6 = conv(cv5, 1024, "conv6")
        cv6 = BatchNormalization()(cv6)
        cv1t = tconv(cv6, 512, "conv1t") + cv5
        cv1t = BatchNormalization()(cv1t)
        cv2t = tconv(cv1t, 256, "conv2t") + cv4
        cv2t = BatchNormalization()(cv2t)
        cv3t = tconv(cv2t, 128, "conv3t") + cv3
        cv3t = BatchNormalization()(cv3t)
        cv4t = tconv(cv3t, 64, "conv4t") + cv2
        cv4t = BatchNormalization()(cv4t)
        cv5t = tconv(cv4t, 32, "conv5t") + cv1
        cv5t = BatchNormalization()(cv5t)
        cv6t = tconv(cv5t, class_nb, "softmax_layer", "softmax")

        argmax_op = otbtf.layers.Argmax(name="argmax_layer")
        
        return {tgt_key: cv6t, "estimated_labels": argmax_op(cv6t)}
