...
class FCNNModel(otbtf.ModelBase):
    ...
    def get_outputs(self, normalized_inputs):
        ...
        cv4 = conv(cv3, 512, "conv4")
        cv4 = BatchNormalization()(cv4)
        cv5 = conv(cv4, 1024, "conv5")
        cv5 = BatchNormalization()(cv5)
        cv1t = tconv(cv5, 512, "conv1t") + cv4
        cv1t = BatchNormalization()(cv1t)
        cv2t = tconv(cv1t, 256, "conv2t") + cv3
        cv2t = BatchNormalization()(cv2t)
        cv3t = tconv(cv2t, 128, "conv3t") + cv2
        cv3t = BatchNormalization()(cv3t)
        cv4t = tconv(cv3t, 64, "conv4t") + cv1
        cv4t = BatchNormalization()(cv4t)
        cv5t = tconv(cv4t, class_nb, "softmax_layer", "softmax")

        argmax_op = otbtf.layers.Argmax(name="argmax_layer")
        
        return {tgt_key: cv5t, "estimated_labels": argmax_op(cv5t)}
