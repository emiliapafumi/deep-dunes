...
def conv(inp, depth, name, strides=2, kernel_regularizer=None):
...
def tconv(inp, depth, name, activation="relu", kernel_regularizer=None):
...
class FCNNModel(otbtf.ModelBase):
    ...
    def get_outputs(self, normalized_inputs):
        ...
        # Add L2 regularization with a regularization factor
        l2_reg = regularizers.l2(0.01)
        
        cv1 = conv(norm_inp_ge, 16, "conv1", kernel_regularizer=l2_reg)
        cv2 = conv(cv1, 32, "conv2", kernel_regularizer=l2_reg) 
        cv3 = conv(cv2, 64, "conv3", kernel_regularizer=l2_reg)
        cv4 = conv(cv3, 64, "conv4", kernel_regularizer=l2_reg)
        cv1t = tconv(cv4, 64, "conv1t", kernel_regularizer=l2_reg) + cv3
        cv2t = tconv(cv1t, 32, "conv2t", kernel_regularizer=l2_reg) + cv2
        cv3t = tconv(cv2t, 16, "conv3t", kernel_regularizer=l2_reg) + cv1
        cv4t = tconv(cv3t, class_nb, "softmax_layer", "softmax", kernel_regularizer=l2_reg)
...
ds_train = create_dataset(
    ["/data/train_ge_patches_v4.tif"],
    ["/data/train_labels_patches_v4.tif"],
)
ds_train = ds_train.shuffle(buffer_size=100)

ds_valid = create_dataset(
    ["/data/valid_ge_patches_v4.tif"],
    ["/data/valid_labels_patches_v4.tif"],
)

ds_test = create_dataset(
    ["/data/test_ge_patches_v4.tif"],
    ["/data/test_labels_patches_v4.tif"],
)
